...
export default function TumorQuotationSession({
  ...
  /* tab 栏只放主面板，跟 DMPK 同一条规矩：**处理过程收进加号菜单，系统不会自己
     把它加回来**。报价结果自带 available（生成后才出现），
     所以生成前实际是两个 tab——和 DMPK 一样。 */
  const [visiblePanelIds, setVisiblePanelIds] = useState<string[]>(["parameters", "rules", "artifacts"]);
  ...
  /* 这一单出过报价没有。
     ...
     「已经出过一版」这个事实当场丢了，于是对话里会把「修改已确认参数」
     说成第一次填。 */
  ...
  /* 参数卡什么时候在：**跟 DMPK 完全一致——收集阶段在，齐了就收起。**
     卡本身默认就是折叠的（分页只列当前这一组），点卡片空白处展开成全屏。
     这条曾经被我加了一个常驻开关（composer 上一颗「填写/收起报价参数」），
     出发点是「参数齐了想回头看一眼没有入口」——但那个入口 DMPK 早就有：
     右栏台账逐项点铅笔改，改哪一项卡就回来列哪一项。
     多一颗按钮换来的是两条业务线形式不一致，而它解决的问题本来就不存在。 */
  const formFields = composerFields;
  ...
  const inspectorPanels = getTumorInspectorPanels({
    ...
    /* 「改这条」把现成的话填进 composer 并聚焦，真正的修改交给对话流
       既有的确认路径——右栏是「看」和「起个头」，不是第二个编辑器。 */
    ...
    onPreviewCost: () => setPreview("artifact"),
  });
