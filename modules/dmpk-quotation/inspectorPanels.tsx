...
function ParametersPanel({ context }: { context: DmpkInspectorContext }) {
  ...
  return <div className="dmpkInspectorList paramCollectList"><header className="paramCollectHead"><span className="paramCollectMark"><FileText size={16} /></span><div><strong>报价参数收集</strong><small>计价参数随对话实时更新</small></div><em>{completed}/{context.fields.length}</em></header><div className="paramCollectProgress" role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100}><i style={{ width: `${pct}%` }} /></div>{hasQuoteDraft ? <section className="dmpkStrategySummary"><header><span><ShieldCheck size={15} /></span><div><strong>报价草稿</strong><small>DMPK 标准价格 · v1.0.13</small></div><em>{adjustmentApplied ? "已调整" : "已计算"}</em></header><p>已按当前参数计算 · 命中 5 项规则</p><footer><button type="button" onClick={() => setDialog("basis")}>查看费用明细</button><button type="button" onClick={() => { setShowAdjustmentPreview(false); setDialog("adjust"); }}>调整本次报价</button></footer></section> : null}{(Object.keys(groupLabels) as DmpkInspectorGroup[]).map((group) => {
