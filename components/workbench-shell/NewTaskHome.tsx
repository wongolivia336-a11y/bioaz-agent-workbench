...
export type QuickStartItem = { id: string; label: string; prompt: string; icon: ReactNode };
...
export function NewTaskHome(props: Props) {
  return <section className="newTaskHome">
    <div className="newTaskIntro"><span className="newTaskMark"><img src="/logo/bioaz-logo.svg" alt="BioAZ" /></span><h1>今天要处理什么？</h1><div className="taskExampleGrid">{props.quickStarts.slice(0, 4).map((item) => <button type="button" key={item.id} onClick={() => props.onTextChange(item.prompt)}>{item.icon}<strong>{item.label}</strong></button>)}</div></div>
    <div className="newTaskComposerDock">
      ...
      {props.pendingRequest && props.suggestedCoworker ? <DispatchConfirmCard coworker={props.suggestedCoworker} coworkers={props.coworkers} project={props.project ?? "临时任务"} onCoworkerChange={props.onCoworkerChange} onConfirm={props.onConfirm} onCancel={props.onCancel} /> : null}
