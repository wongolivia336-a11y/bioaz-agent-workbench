...
export default function WorkbenchShell() {
  ...
  const lensAccounts = activeLens.accountIds.length
    ...
    : inboxAccounts;
  /* 首屏的快捷入口跟侧栏、收件箱用同一把尺子。演示 DMPK 那条线时,首屏摆着
     两条跟它无关的业务线,甲方第一眼看到的就不是他要看的东西。
     **脚手架,上线前跟 demoLens 一起删。** */
  const lensQuickStarts = activeLens.moduleId
    ? quickStarts.filter((item) => item.moduleId === activeLens.moduleId)
    : quickStarts;
