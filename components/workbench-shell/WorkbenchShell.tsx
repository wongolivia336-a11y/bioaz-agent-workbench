...
import { quotePermissionsFor } from "../../lib/workbench/permissions";
...
export default function WorkbenchShell() {
    ...
    : libraryView === "outputs"
      ...
      : libraryView === "trash"
        ...
        : libraryView === "folder"
          ...
          : null;
  /* 报价管理后台盖在工作台上面，工作台不卸载。
     ----------------------------------------------------------------------
     原来是 `if (quotationTarget) return <QuotationManagement/>`——整个工作台树连同
     会话一起卸掉，从后台「返回工作台」回来，对话、参数、临时价全没了，像刷新了一样。
     会话的状态都在 Session 组件里（它就该在那儿），所以后台开着的时候把工作台藏起来
     而不是拆掉：display:none 用行内样式写，.dmpkShell 那一堆 display:grid 才压不过它。 */
  /* 账号级别推出来的报价权限：板块里能不能改价、能不能看完整价目表、后台能不能改底表。
     演示级——左下角切一下账号，同一单在 SD 和 SD 助理眼里各是什么样。 */
  const quotePermissions = quotePermissionsFor(account);
  const backOffice = quotationTarget
    ? <QuotationManagement onBack={() => setQuotationTarget(null)} initialBusiness={quotationTarget.business} initialTab={quotationTarget.tab} initialDraft={quotationTarget.draft} readOnly={!quotePermissions.canEditCatalog} viewerGrade={account.grade} />
    : null;
  const shellHiddenStyle = quotationTarget ? { display: "none" } : undefined;
  /* 外壳类名：自己的 + 借来的。
     ----------------------------------------------------------------------
  ...
  const shellClasses = activeModule
    ...
    : "";
  return <>{backOffice}<main className={`dmpkShell ${collapsed ? "sidebarCollapsed" : ""} ${shellView ? "workbenchShell" : "moduleSessionShell"} ${shellClasses}`} style={shellHiddenStyle} aria-hidden={quotationTarget ? true : undefined}>
