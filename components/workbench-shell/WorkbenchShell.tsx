...
export default function WorkbenchShell() {
  ...
  const quickStarts = useMemo(() => quickStartRegistry.map((item) => { const Icon = item.icon; const coworker = getAgentModule(item.moduleId)?.suggestedCoworker; return { id: item.id, label: item.label, prompt: item.prompt, availability: item.availability, moduleId: item.moduleId, coworkerId: coworker?.id, coworkerName: coworker?.name, icon: <Icon size={16} /> }; }), []);
  ...
  const spaceStats = useMemo(() => {
    ...
    const artifactKinds = new Set(["交付产物", "报告图表", "审核记录", "计算依据"]);
    ...
    return {
      files: files.filter((file) => !artifactKinds.has(file.kind)).length,
      artifacts: files.filter((file) => artifactKinds.has(file.kind)).length,
      tasks: tasks.length,
    };
