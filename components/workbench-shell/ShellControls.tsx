      ...
      {open ? (
        <section className={`workspaceAssistantPanel ${library ? "libraryAssistantWorkspace" : ""}`} aria-label={library ? "BioAZ 资料助手" : "BioAZ Helper"}>
          ...
          <div className="workspaceAssistantBody">
            {question ? <><div className="assistantExchange"><p>{question}</p><div><Sparkles size={14} /><span>{library ? "我找到了 4 项相关内容，已结合当前项目与文件上下文整理。" : "已按最近更新时间检查任务，顶部三项需要你处理。"}</span></div></div>{library ? <div className="assistantReferences"><button type="button"><FileSearch size={16} /><span><strong>样本9_双批次报告_v3.docx</strong><small>XX药业-PD1临床前评价 · 36分钟前</small></span></button><button type="button"><BookOpen size={16} /><span><strong>肿瘤药效统计口径_v4.xlsx</strong><small>规则与模板 · 已发布</small></span></button><button type="button"><FolderKanban size={16} /><span><strong>样本9_双批次交付包.zip</strong><small>报告、图表、QC 与证据摘要</small></span></button></div> : null}</> : <div className="assistantWelcome"><span className="assistantHeroMark"><img src="/logo/bioaz-logo.svg" alt="" /></span><strong>{library ? "需要找什么资料？" : "你好，我是 BioAZ Helper"}</strong><p>{library ? "我会结合当前项目、筛选条件和已选文件查找资料。" : "找任务、查进度或发起新工作。"}</p></div>}
            <div className="assistantSuggestions">{suggestions.map((item, index) => <button type="button" key={item} onClick={() => submit(item)}>{library ? [<FileSearch key="search" size={16} />, <BookOpen key="rule" size={16} />, <FolderKanban key="folder" size={16} />][index] : null}<span>{item}</span></button>)}</div>
          </div>
          <form className="workspaceAssistantComposer" onSubmit={(event) => { event.preventDefault(); submit(text); }}><input value={text} onChange={(event) => setText(event.target.value)} placeholder={library ? "询问项目文件、规则或产物..." : "给 BioAZ Helper 发消息..."} aria-label={library ? "给 BioAZ 资料助手发消息" : "给 BioAZ Helper 发消息"} /><button type="submit" aria-label="发送" disabled={!text.trim()}><Send size={16} /></button></form>
        </section>
      ) : null}
