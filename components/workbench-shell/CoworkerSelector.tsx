    ...
    <div ref={menuRef} className={`roleSelector ${open ? "isOpen" : ""}`}>
      ...
      {open ? (
        <div className="roleSelectorMenu isOpen">
          ...
          {locked && !forceMode ? <button className="roleSelectorForceSwitch" type="button" onClick={() => setForceMode(true)}><ArrowRightLeft size={14} /><strong>结束当前流程并切换</strong><span>需要再次确认</span></button> : null}
        </div>
      ) : null}
    </div>
  );
