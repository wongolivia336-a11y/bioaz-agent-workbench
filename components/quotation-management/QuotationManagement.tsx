...
import { ArrowLeft, Bot, Check, ChevronRight, ChevronUp, FileSearch, FileSpreadsheet, FileText, FlaskConical, GitBranch, GripVertical, Lightbulb, ListChecks, LogOut, Orbit, Plus, Search, Send, Settings, Settings2, Sparkles, X } from "lucide-react";
...
export function QuotationManagement({ onBack }: { onBack: () => void }) {
  ...
  const assistantExpanded = assistantHovered || assistantLocked;
  ...
  return <main className="quotationManagementShell">
    ...
    {business === "dmpk" ? <div
      ...
      data-expanded={assistantExpanded || assistantOpen ? "true" : "false"}
      onMouseEnter={() => setAssistantHovered(true)}
      onMouseLeave={() => setAssistantHovered(false)}
      onFocusCapture={() => setAssistantLocked(true)}
      onBlurCapture={() => requestAnimationFrame(() => {
        if (!assistantRef.current?.contains(document.activeElement) && !assistantText.trim()) setAssistantLocked(false);
      })}
    >
      {!assistantOpen ? <div className="ambientAssistantSuggestions" aria-hidden={!assistantExpanded}>
        <button type="button" tabIndex={assistantExpanded ? 0 : -1} onClick={() => { setAssistantText("以后 PK 样品少于 40 个都按 40 个收费"); setAssistantOpen(true); }}><GitBranch /><span>改规则</span></button>
        <button type="button" tabIndex={assistantExpanded ? 0 : -1} onClick={() => { setAssistantText("给 PK 检测增加一个自定义采血时间点字段"); setAssistantOpen(true); }}><ListChecks /><span>改字段</span></button>
        <button type="button" tabIndex={assistantExpanded ? 0 : -1} onClick={() => { setAssistantText("调整 PK 报价单模板里的费用明细顺序"); setAssistantOpen(true); }}><FileText /><span>改模板</span></button>
      </div> : null}
      <div className="ambientComposerFrame">
        {!assistantOpen ? <button className="ambientAssistantEntry" type="button" aria-label="展开 DMPK 报价同事" aria-expanded={assistantExpanded} onClick={() => { setAssistantLocked(true); setAssistantOpen(true); }}><Sparkles size={17} /><span>问 DMPK 报价同事</span></button> : null}
        {assistantOpen ? <section className="quotationAssistantPanel">
          <header><div><span><Bot size={15} /></span><strong>DMPK 报价同事</strong></div><button type="button" onClick={() => { setAssistantOpen(false); setAssistantProposal(null); setAssistantLocked(false); }} aria-label="关闭 DMPK 报价同事"><X size={15} /></button></header>
          {assistantProposal ? <div className="quotationChangeProposal"><span className="quotationAiTag"><Sparkles size={13} />已识别修改对象</span><h3>这次将修改计价规则</h3><dl><div><dt>修改</dt><dd>计价规则</dd></div><div><dt>不会修改</dt><dd>报价字段、报价模板</dd></div><div><dt>影响范围</dt><dd>未来新建的 PK 报价</dd></div></dl><footer><button type="button" onClick={() => setAssistantProposal(null)}>返回修改</button><button className="primary" type="button" onClick={() => { setRuleDraft(assistantProposal); setTab("rules"); setAssistantOpen(false); setAssistantProposal(null); setAssistantLocked(false); }}>确认并进入规则编辑</button></footer></div> : <form className="ambientAssistantComposer quotationAssistantComposer" onSubmit={(event) => { event.preventDefault(); if (assistantText.trim()) setAssistantProposal(assistantText.trim()); }}><label className="ambientComposerAdd" aria-label="添加文件"><Plus size={18} /><input type="file" multiple /></label><input autoFocus value={assistantText} onChange={(event) => setAssistantText(event.target.value)} placeholder="描述要改的字段、规则或模板..." /><button className="ambientComposerSend" type="submit" disabled={!assistantText.trim()} aria-label="发送"><Send size={17} /></button></form>}
        </section> : null}
      </div>
    </div> : null}
