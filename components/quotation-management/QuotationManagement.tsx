...
function RulesView() {
  const [prompt, setPrompt] = useState(() => typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("draft") || "SD 大鼠超过 30 只时，动物使用费按 85 折计算" : "SD 大鼠超过 30 只时，动物使用费按 85 折计算");
  ...
  return <><section className="quotationRulePrompt"><textarea value={prompt} onChange={(event) => { setPrompt(event.target.value); setGenerated(false); setShowTrial(false); setSaved(false); setPublished(false); }} aria-label="描述特殊报价规则" placeholder="例如：SD 大鼠超过 30 只时，动物使用费按 85 折计算" /><footer><button className="primary" type="button" disabled={!prompt.trim()} onClick={() => { setGenerated(true); setShowTrial(false); }}><Sparkles size={15} />整理为规则</button></footer></section>{generated ? <div className="quotationRuleGrid"><article className="quotationRuleCard"><header className="quotationRuleDraftHeader"><span className="quotationAiTag"><Bot size={13} />{published ? "已发布" : saved ? "草稿已保存" : "规则草稿"}</span><small>{published ? "将用于后续新报价" : "尚未生效"}</small></header><h2>SD 大鼠批量折扣</h2><RuleSentence prefix="适用于" values={["PK 检测"]} /><RuleSentence prefix="当" values={["动物种属", "等于", "SD 大鼠"]} /><RuleSentence prefix="并且" values={["动物数量", "大于", "30", "只"]} />{extraCondition ? <RuleSentence prefix="并且" values={["试验周期", "小于等于", "4", "周"]} /> : null}<RuleSentence prefix="调整为" values={["动物使用费", "乘以", "85%"]} />{extraAdjustment ? <RuleSentence prefix="同时" values={["报告费", "设为", "2500", "元"]} /> : null}<div className="quotationRuleAddArea"><div className="quotationRuleAddActions"><button type="button" onClick={() => openInlineEditor("condition")}><Plus size={14} />添加条件</button><button type="button" onClick={() => openInlineEditor("adjustment")}><Plus size={14} />添加费用调整</button></div>{inlineEditor ? <section className="quotationInlineRuleEditor"><header><div><Sparkles size={14} /><strong>{inlineEditor === "condition" ? "新增触发条件" : "新增费用调整"}</strong></div><button type="button" onClick={() => setInlineEditor(null)} aria-label="关闭"><X size={14} /></button></header><textarea autoFocus rows={2} value={inlineRequest} onChange={(event) => setInlineRequest(event.target.value)} /><footer><small>AI 会整理为可继续编辑的规则字段</small><button className="primary" type="button" disabled={!inlineRequest.trim()} onClick={addInlineRule}>整理并添加</button></footer></section> : null}</div><footer><span className="quotationRuleSaveHint">{published ? "规则已生效" : saved ? "草稿已保存，尚未生效" : "修改尚未保存"}</span><div><button type="button" onClick={() => setSaved(true)}>保存草稿</button><button type="button" onClick={() => setShowTrial(true)}>验证计算</button><button className="primary" type="button" disabled={!showTrial} onClick={() => { setSaved(true); setPublished(true); }}>发布规则</button></div></footer></article>{showTrial ? <aside className="quotationTrial"><h2>验证结果</h2><small>PK · SD 大鼠 · 36 只</small><strong className="quotationTrialStatus"><Check size={14} />规则已触发</strong><dl><div><dt>标准动物费</dt><dd>¥4,320</dd></div><div><dt>规则调整</dt><dd>− ¥648</dd></div>{extraAdjustment ? <div><dt>报告费</dt><dd>¥2,500</dd></div> : null}<div><dt>调整后动物费</dt><dd>¥3,672</dd></div></dl></aside> : null}</div> : <div className="quotationRuleEmpty"><Sparkles size={18} /><strong>描述一条例外计价方式</strong><span>AI 会整理成可编辑的规则草稿。</span></div>}</>;
}

function RuleSentence({ prefix, values }: { prefix: string; values: string[] }) { return <div className="quotationRuleSentence"><span>{prefix}</span><div>{values.map((value, index) => <EditableRuleToken initialValue={value} key={`${value}-${index}`} />)}</div></div>; }

function EditableRuleToken({ initialValue }: { initialValue: string }) {
  const [value, setValue] = useState(initialValue);
  const [editing, setEditing] = useState(false);
  return editing ? <input className="quotationRuleTokenInput" autoFocus value={value} onChange={(event) => setValue(event.target.value)} onBlur={() => setEditing(false)} onKeyDown={(event) => { if (event.key === "Enter") setEditing(false); }} aria-label={`修改${initialValue}`} /> : <button type="button" title="点击修改" onClick={() => setEditing(true)}>{value}</button>;
}
...
function TemplatesView({ onView }: { onView: () => void }) {
  ...
  return <div className="quotationTemplateWorkspace"><div className="quotationTemplateScenarioGrid">{templates.map((template) => <article className={template.id === activeTemplate ? "active" : ""} key={template.id} onClick={() => setActiveTemplate(template.id)}><header><span><FileSpreadsheet size={18} /></span><em><Check size={12} />当前使用</em></header><strong>{template.title}</strong><small>当前版本 {template.version} · Excel 与 Word</small><footer><button type="button" onClick={(event) => { event.stopPropagation(); setActiveTemplate(template.id); onView(); }}>预览</button><button type="button" onClick={() => setActiveTemplate(template.id)}>版本记录 <ChevronRight size={14} /></button></footer></article>)}</div><section className="quotationTemplateHistory"><header><div><h2>{selected.title}</h2><small>历史版本</small></div><span>共 {selected.count} 个版本</span></header>{[[selected.version, "当前使用", selected.date], [selected.id === "pk" ? "v7" : selected.id === "ba" ? "v3" : "v1", "历史版本", "2026-06-20"], [selected.id === "pk" ? "v6" : "—", "历史版本", "2026-05-08"]].filter((item) => item[0] !== "—").map(([version, status, date]) => <button type="button" key={version} onClick={onView}><strong>{version}</strong><span>{status}</span><small>{date}</small><ChevronRight size={15} /></button>)}</section></div>;
}
